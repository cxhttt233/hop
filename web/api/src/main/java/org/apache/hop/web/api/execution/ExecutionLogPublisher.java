...
public final class ExecutionLogPublisher<T extends PipelineMeta> implements AutoCloseable {
  ...
  public ExecutionLogPublisher(ExecutionRegistry.Entry<IPipelineEngine<T>> entry) {
    this.entry = entry; this.rootLogChannelId = entry.execution().getLogChannelId(); this.listener = this::onEvent;
  }
  public void start() { HopLogStore.getAppender().addLoggingEventListener(listener); }
  private void onEvent(HopLoggingEvent event) {
    if (!(event.getMessage() instanceof LogMessage message) || !belongsToExecution(message)) return;
    entry.events().append("log", new LogEvent(event.timeStamp, event.getLevel().getCode(), message.getSubject(), message.getMessage()));
  }
  ...
  @Override public void close() { HopLogStore.getAppender().removeLoggingEventListener(listener); }
