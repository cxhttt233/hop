...
public final class ExecutionEventBuffer {
  ...
  public ExecutionEventBuffer(int capacity) { this(capacity, Clock.systemUTC()); }
  ExecutionEventBuffer(int capacity, Clock clock) {
    if (capacity < 1) throw new IllegalArgumentException("capacity must be positive");
    this.capacity = capacity; this.clock = clock; this.events = new ArrayDeque<>(capacity);
  }
  public synchronized ExecutionEvent append(String type, Object payload) {
    ...
    events.addLast(event); return event;
  }
  public synchronized List<ExecutionEvent> replayAfter(long lastEventId) {
    List<ExecutionEvent> replay = new ArrayList<>();
    for (ExecutionEvent event : events) if (event.seq() > lastEventId) replay.add(event);
    return List.copyOf(replay);
  }
  public synchronized long latestSequence() { return sequence; }
  public synchronized long oldestRetainedSequence() { return events.isEmpty() ? sequence + 1 : events.getFirst().seq(); }
}
