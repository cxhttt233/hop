...
## 0A. 2026-09-24 产品目标修订（优先级高于本文后续冲突内容）

### 0A.1 最终目标

本项目的首要目标不是“重构 Hop Web 后端”，而是：

> **保留 Apache Hop 的 Java 引擎、模型、插件与 `.hpl/.hwf` 兼容能力，把 SWT/RAP 表现层逐步替换为 React 原生现代浏览器 UI。**

因此判断任何工作优先级时，首先问：**它是否缩短“完全不进入 RAP /ui 即可日常编辑 Hop”的路径。**

### 0A.2 仓库边界

推荐建立独立产品仓库 **`cxhttt233/hop-modern-web`**，不复制 Apache Hop 全量源码。

独立仓库只保存：
- React + TypeScript SPA；
- React Flow 画布与现代 Web 组件；
- 薄 JAX-RS / Jakarta 服务端适配层；
- Graph / Config / Metadata Web 契约；
- 端到端与契约测试；
- GitHub Actions；
- 本项目架构、状态和迁移说明。

现有 **`cxhttt233/hop`** fork 继续作为：
- Apache Hop 引擎基线与兼容验证源；
- 只有确实必须下沉到 `core/engine` 的小型、可上游化 patch 的承载仓库；
- 不再作为现代 Web 产品代码的长期主仓库。

禁止把 `core/`、`engine/`、`ui/`、`rap/`、`plugins/` 整体复制进新仓库。开发/CI 通过固定 commit 的 Hop fork/Apache Hop 构建所需 artifacts；必要 engine patch 仍留在 Hop fork。

### 0A.3 前端技术选择

PoC 起固定：
- **React + TypeScript**
- **React Flow** 作为 Pipeline/Workflow 浏览器原生图编辑器
- Monaco 用于 SQL / JS / JSON / Markdown
- 组件库保持可替换，业务契约不绑定具体 UI library

Vue 3 不再作为并行实施选项，除非以后明确重新决策。

### 0A.4 Canvas 主路径修订

本文后续“服务端权威 SVG 渲染 + 客户端交互层”不再是主 Canvas 路径。
...
React Flow
        ↓
浏览器原生节点 / Hop / Note / Selection / Drag / Connect
        ↓
Semantic Commands
        ↓
Java Document Model
...
原则：
- Java 仍是文档模型、字段推导、校验、插件语义和保存的权威。
- React 负责视觉呈现与交互状态，不复制 Hop 引擎业务语义。
- `PipelineCanvasSvgRenderer` / `WorkflowCanvasSvgRenderer` 保留为兼容、回归对照、特殊 Painter extension fallback 和测试参考，不再决定主 UI。
- Graph JSON 必须包含足够的节点、hop、note、位置、插件标识、状态与前端渲染所需信息。
- 后续若某个 Painter extension 无法立即映射到 React，可降级到兼容提示/fallback，不得因此把整个 Canvas 主路径退回服务端 SVG。

### 0A.5 Phase 0 的唯一核心验收

Phase 0 优先完成以下垂直切片：

1. 在 **React `/app`** 中运行，不实例化 SWT/RWT UI；
2. 打开真实 `.hpl`；
3. Graph JSON → React Flow 显示真实 Pipeline；
4. 浏览器完成 select / pan / zoom / move / add / delete / connect / undo；
5. 打开至少一个真实 Transform 的 React 原生配置界面；
6. 首选 Table Input，验证：
   - MetadataPicker
   - SQL Monaco
   - Variables
   - Config JSON round-trip
7. 再覆盖 Select Values 与 Sort Rows，验证 EditableTable / Get Fields 等共性；
8. 至少一个高频 Metadata 类型（优先 Database Connection）可在 React 原生 Metadata Editor 中查看、编辑和保存；
9. 保存 `.hpl`，重新加载后语义一致，原 Apache Hop 仍可正常打开和执行。

**在该垂直切片出现之前，Execution/SSE/Metrics 的进一步完善、复杂多用户增强、深层 Project/Environment/VFS 基础设施和插件 Web UI 长尾均降级为后续工作。**

### 0A.6 资产保留与暂停

保留并继续利用：
- WebSessionScope / 最小 session 隔离；
- Document commands / undo；
- Config JSON / Schema；
- 已完成的 Execution domain 代码作为 deferred validated asset；
- 现有 Hop 引擎与 plugin metadata。

优先继续：
- Graph Projection JSON；
- command → save → reload；
- Schema → React components；
- Metadata CRUD / picker 所需最小契约；
- React `web/app`。

暂停继续扩展：
- Execution SSE/reconnect/metrics 的完整化；
- 不被当前垂直切片使用的 Session/Project/Environment/VFS 深层抽象；
- 服务端 SVG 作为主 Canvas；
- 为“以后可能需要”而提前铺设的基础设施。

### 0A.7 开发与验证环境

- 代码修改：GitHub repository / connector。
- 构建、格式、测试、E2E：GitHub Actions。
- 不使用 Desktop Commander、Remote Desktop 或远程开发机作为本项目开发依赖。
- standalone repo 的 Actions 可 checkout 固定 Hop fork commit 并只构建/安装必要 Maven artifacts；无需把 Hop 源码保存在 standalone repo。
